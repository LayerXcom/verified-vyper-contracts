...
N_LIMBS_LENGTH: constant(uint256) = 8
...
def _modularExp(_base: uint256[8], _e: uint256, _m: uint256[8]) -> uint256[8]:
    ...
    exponent: bytes32 = convert(_e, bytes32)
    modulus: bytes[256] = self._convertUInt256ListToBytes(_m)
    # ref. https://eips.ethereum.org/EIPS/eip-198
    # 640 = 32 * 3 + <length_of_BASE> + <length_of_EXPONENT> + <length_of_MODULUS>
    data: bytes[640] = concat(BASE_BYTE_COUNT_BYTES32, convert(32, bytes32), M_BYTE_COUNT_BYTES32,
                    base, exponent, modulus)
    # FIXME: gas?
    # NOTE: Fix this when a static call is supported in Vyper
    return self._convertBytesArrayToUInt256List(raw_call(PRECOMPILED_BIGMODEXP, data, outsize=256, gas=2000))
...
def checkNonInclusionProof(_primes: uint256[8], _rLimbs: uint256[8], _cofactorLimbs: uint256[8]) -> bool:
    ...
    rhs: uint256[8] = self._modularExpVariableLength(self._initialAccumulator(), _cofactorLimbs, Nread)
